// app/(app)/debate-review.tsx
//
// Review screen for a saved Debate session, opened from /history. Forked from
// explain-review.tsx; reuses ImpromptuReviewSkeleton (same body shape) and adds a
// statement + stance context card above the AI feedback. Only the mode it loads/guards
// on and the default title differ.
...
// Debate's review body has the same shape as impromptu's, so the impromptu skeleton fits.
...
type DebateSession = Extract<LoadedSession, { mode: 'debate' }>;

export default function DebateReview() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        console.warn('[debate-review] load failed:', e);
  ...
  const displayTitle = (session ? session.customTitle : title) ?? DEFAULT_TITLE.debate;
    ...
    >
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        ...
        {loading ? (
          ...
          <DebateReviewBody session={session} />
        ) : null}
      </SafeAreaView>
...
function DebateReviewBody({ session }: { session: DebateSession }) {
  const { data } = session;
    ...
    >
      {metrics.tooShort ? (
        ...
        <View style={styles.contentCard}>
          ...
          {data.statement ? <DebateContextCard statement={data.statement} stance={data.stance} /> : null}
