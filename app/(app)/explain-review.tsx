// app/(app)/explain-review.tsx
//
// Review screen for a saved Explain session, opened from /history. Forked from
// impromptu-review.tsx — the body is byte-for-byte identical (AI feedback, the four
// metric rows, transcript, audio), so it reuses ImpromptuReviewSkeleton too. The only
// differences are the mode it loads/guards on and the default title.
...
// The Explain review body has the same shape as impromptu's (feedback + 4 rows +
...
type ExplainSession = Extract<LoadedSession, { mode: 'explain' }>;
...
export default function ExplainReview() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        console.warn('[explain-review] load failed:', e);
  ...
  const displayTitle = (session ? session.customTitle : title) ?? DEFAULT_TITLE.explain;
    ...
    >
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        ...
        {loading ? (
          ...
          <ExplainReviewBody session={session} />
        ) : null}
      </SafeAreaView>
...
function ExplainReviewBody({ session }: { session: ExplainSession }) {
  const { data } = session;
