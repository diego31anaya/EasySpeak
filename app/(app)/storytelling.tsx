// storytelling.tsx
//
// The "Storytelling" practice flow — a batch record → done → feedback loop. Forked
// from explain.tsx: the user optionally types WHAT the story is about, records
// themselves telling it, hits Stop, and gets AI feedback on the story's craft (arc,
// scene, momentum, landing) plus the usual delivery metrics. NO prompt generation,
// NO TTS. Feedback is hardcoded to the storytelling focus (the mode IS the focus).
...
import { buildStorytellingFeedbackInput, generateStorytellingFeedback } from '../../lib/storytelling-feedback';
...
import {
  saveStorytellingSession as saveStorytellingHistory,
  setPendingSaveId,
...
const WAVE_BARS = 40;
// Cap the typed topic — it's a short subject line, not an essay, and it becomes the
// session title.
const TOPIC_MAX = 80;
...
export default function Storytelling() {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      try {
        if (!metrics.tooShort) {
          const fb = await generateStorytellingFeedback(
            // Feedback is hardcoded to the storytelling focus — the mode IS the focus,
            // so it weights expressive delivery regardless of the user's profile focus.
            buildStorytellingFeedbackInput(
              storyTopic,
              result.transcript,
              metrics,
              finalDuration,
              focusGuidance('storytelling'),
            ),
          );
          console.log('[Storytelling Feedback]', fb.feedback);
          console.log('[Storytelling Feedback] score:', fb.score);
          aiFeedback = fb.feedback;
          aiScore = fb.score;
        }
      } catch (err) {
        console.warn('[Storytelling Feedback] Failed:', err);
  ...
  const handleReset = () => {
    ...
    setFrozenElapsed(null);
    setPhase('idle');
    // Keep the typed topic so a retry doesn't lose it.
    r.reset();
  };
    ...
    >
      ...
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        <View style={styles.header}>
          ...
          {phase === 'idle' && (
            // PLACEHOLDER copy.
            <Text style={styles.subtitle}>Record yourself telling a story, then get feedback.</Text>
          )}
        </View>
          ...
          <>
            ...
            >
              <View style={styles.section}>
                <Text style={styles.eyebrow}>What's the story about?</Text>
                <View style={styles.inputWrap}>
                  <TextInput
                    ...
                    // PLACEHOLDER copy.
                    placeholder="e.g. the time I got lost in Tokyo (optional)"
            ...
            <View style={styles.footer}>
              <Pressable
                onPress={handleStart}
                style={({ pressed }) => [pressed && styles.btnPrimaryPressed]}
              >
                <LinearGradient
        ...
        {phase === 'analyzing' && (
          <View style={styles.stage}>
            ...
            <View style={styles.stageStatus}>
              <AiOrbLoading />
              <Text style={[styles.statusText, styles.loadingStatusText]}>Analyzing your story</Text>
            </View>
          </View>
        )}
