// app/(app)/explain-results.tsx
//
// Results screen for a freshly finished Explain practice run. Data arrives via nav
// params from explain.tsx; this screen deserializes the metrics and renders feedback
// + the four metric rows + transcript + audio, with a "Try again" footer. Forked from
// impromptu-results.tsx — the only differences are the param names, the topic-as-title
// seed, and the mode defaults. Reviewing a saved session lives on explain-review.tsx.
...
export default function ExplainResults() {
    const params = useLocalSearchParams<{transcript: string; words: string; durationSec: string; explainTopic?: string; audioUri?: string; aiFeedback?: string; aiFeedbackError?: string; aiScore?: string; metrics?: string; }>();
    ...
    // The topic (generated or the user's own) is saved to the `prompt` column, not the
    // title — like debate/prep — so the header stays the generic DEFAULT_TITLE.explain and
    // the topic shows in the "Topic" card below. Rename still works (custom_title), gated on
    // sessionId like the favorite star (the background save must land before it can persist).
    const initialTopic = (params.explainTopic ?? '').trim();
    const [customTitle, setCustomTitle] = useState<string | null>(null);
    const [renameOpen, setRenameOpen] = useState(false);
    const displayTitle = customTitle ?? DEFAULT_TITLE.explain;
    ...
    return (
        ...
        <RenameSheet
            ...
            placeholder={DEFAULT_TITLE.explain}
