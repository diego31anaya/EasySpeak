// app/(app)/impromptu-review.tsx
//
// Review screen for a saved Impromptu session, opened from /history. It takes a
// `sessionId`, fetches the session itself (getSession), shows a skeleton while
// loading, then renders the same body as impromptu-results — AI feedback, the
// four metric rows, transcript, and audio. Header is a back chevron + centered
// title (same shape as /history), which pops back to the list. A quiet "Delete
// Session" sits at the bottom of the scroll, below the body (review-only — see
// ReviewDeleteButton); there's no "Try again" (nothing to retry from a saved
// session) and no defer dance: the push lands on this light screen, the heavy
// content mounts against the skeleton, so there's no row sweep to protect.
...
type ImpromptuSession = Extract<LoadedSession, { mode: 'impromptu' }>;

export default function ImpromptuReview() {
  const { sessionId, title } = useLocalSearchParams<{ sessionId: string; title?: string }>();

  const [session, setSession] = useState<ImpromptuSession | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (!loaded || loaded.mode !== 'impromptu') {
          setSession(null);
        ...
        if (cancelled) return;
        console.warn('[impromptu-review] load failed:', e);
  ...
  const displayTitle = (session ? session.customTitle : title) ?? DEFAULT_TITLE.impromptu;
    ...
    >
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        ...
        {loading ? (
          ...
          <ImpromptuReviewBody session={session} />
        ) : null}
      </SafeAreaView>
...
function ImpromptuReviewBody({ session }: { session: ImpromptuSession }) {
  const { data } = session;
  const audioRef = useRef<AudioPlaybackHandle | null>(null);

  // data.metrics is the serialized form (Sets → arrays); rehydrate it here, the
  // same way the practice screen does from nav params.
    ...
    >
      {metrics.tooShort ? (
        ...
        <View style={styles.contentCard}>
          ...
          {data.impromptuPrompt ? (
            <PromptContextCard label="Prompt" text={data.impromptuPrompt} />
          ) : null}
